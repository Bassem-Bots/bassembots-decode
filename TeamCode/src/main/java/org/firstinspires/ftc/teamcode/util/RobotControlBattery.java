...
public class RobotControlBattery {
    ...
    private static final double TICKS_PER_REV = 112.0;  // ADJUST THIS VALUE - start with 112, increase if motor runs too fast
    ...
    public RobotControlBattery(LinearOpMode opmode) {
        myOpMode = opmode;
    }
    ...
    public void init() {
        ...
        // Set PIDF coefficients for velocity control (adjust these values if needed)
        // These are typical starting values - tune based on your motor's response
        PIDFCoefficients pidfCoefficients = new PIDFCoefficients(
            15.0,  // P - Proportional gain
            0.0,   // I - Integral gain
            0.0,   // D - Derivative gain
            13.5   // F - Feed-forward gain (typically motor-specific)
        );
...
//        armTarget = (int) (lirililarila*1000);
    ...
    public void setShooterPower(ShooterPower shooterPower) {
        setShooterVelocity(shooterPower.getPower());
    }
